...
class Graph // 有向无环图
{
    ...
    public:
         ...
         *  1. 构建逆邻接表
         *  2. 递归处理每一个还没有被访问的节点
         *      具体做法很简单：对于一个节点来说，先输出指向它的所有顶点，再输出自己
         *  3. 如果这个顶点还没有被遍历过，就递归遍历它，把所有指向它的节点都输出了，再输出自己
         *      注意：当访问一个节点的时候，应当先递归访问它的前驱节点，直至前驱节点没有前驱节点为止
         */
        void topoSortByDFS()
        {
            /*
                先构建逆邻接表，边 s->t表示，s依赖于t, t先于s
                这里使用逆邻接表。其实就是检测这个有向图中有没有环，只要存在环，图中的顶点就不能遍历完 
            */

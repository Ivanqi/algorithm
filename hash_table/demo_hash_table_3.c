...
int hashtab_insert(hashtab* h, void *key, void *data) {
    ...
    if ((h == NULL) || (key == NULL) || (data == NULL)) return 1;

    // 获取hash 数值
    hvalue = h->hash_value(h, key);
    cur = h->htables[hvalue];

    // hash 桶中元素是从小到大排列的，找到要插入的位置
    while ((cur != NULL) && (h->keycmp(h, key, cur->key) > 0)) {
        prev = cur;
        cur = cur->next;
    }

    // 如果key和当前key对比一致，直接返回,数据已经存在
    if ((cur != NULL) && (h->keycmp(h, key, cur->key) == 0)) return 2;

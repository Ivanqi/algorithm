...
// 数据存放节点
typedef struct __linked_hash_map_node {
    void *key;  // 键
    void *data; // 数据
    struct __linked_hash_map_node *next;    // 哈希冲突时，用来挂接后续节点
...
typedef struct _linked_hash_map {
    ...
    int size;
    int nel_max;    // 支持最大节点数
    int (*hash_value)(struct __linked_hash_map *h, const void *key);    // 哈希函数
    int (*keycmp)(struct __linked_hash_map *h, const void *key1, const void key2);  // 哈希key比较函数,当哈希值一致时使用
...
LinkedHashMapNode *LinkedHashMapNode_delet(LinkedHashMap *h, void *key);
#endif